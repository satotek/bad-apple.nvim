...
function Player:create_buffer()
  ...
  local map_options = { buffer = buffer, silent = true }
  ...
  vim.schedule(function()
    ...
    for _, patch in ipairs(patches) do
      ...
      if patch.row >= line_count then
        vim.api.nvim_buf_set_lines(self.buffer, line_count, -1, false, vim.fn['repeat']({ "" }, patch.row - line_count + 1))
      end
